...
import { UsersEP, UtilsEP } from '../../services/api/routes';
...
import { IResetPasswordPayload, IExistsPayload } from '../../types/interfaces';
...
import { setUserIdToDeepLink } from '../actions/deepLinksActions';
import { updateRegistrationForm, resetRegistrationForm } from '../actions/registrationActions';
import { errorHandler } from '../../utils';
import { deepLinksSelector } from '../selectors/deepLinksSelector'
import authApi from '../../services/api/authInstance';
import { resetOpportunities } from '../actions/opportunities';
...
interface IForgotPasswordPayload {
	payload: { email: string, cb?: (response: any) => void };
}
...
function* loginSaga({ payload }: ILoginPayload) {
	try {
		const { accessToken, refreshToken,isEmailRegistered } = yield Auth.login(
			payload.email.trim(),
		...
		if(!isEmailRegistered){
			navigationService.navigate(ScreensEnum.CHECK_EMAIL_SCREEN,{email:payload.email.trim()});
		}
		yield put(setEmailValidation(isEmailRegistered))
	} catch (ex) {
		yield put(loginError());
		yield put(loginField());
		//navigationService.navigate(ScreensEnum.TRY_AGAIN_OR_SIGN_UP_MODAL);
		setCrumbizApiAuthorizationHeader(null);
	}
}
function* getExistsSaga({ payload }: IExistsPayload) {
	try {
		const { email, phone } = payload
		let emailExist = yield call(UtilsEP.checkUserExistence, { email })
		let phoneExist = false
		if (phone) {
			phoneExist = yield call(UtilsEP.checkUserExistence, { phone })
		}
		...
		if (!emailExist && !phoneExist) {
			...
			if (emailExist) {
				if ((emailExist[0].AuthId == "00000000-0000-0000-0000-000000000000")) {
					yield put(setUserIdToDeepLink(emailExist[0].Id, true))
					emailExist = false
				}
			}
			if (phoneExist) {
				if ((phoneExist[0].AuthId == "00000000-0000-0000-0000-000000000000")) {
					yield put(setUserIdToDeepLink(phoneExist[0].Id, false))
					phoneExist = false
				}
			}

		}
		yield put(setExists({ emailExist: !!emailExist, phoneExist: !!phoneExist }))
	} catch (ex) {
...
function* registerSaga({ payload }: IRegisterPayload) {
	try {
		...
		yield put(updateRegistrationForm({ ...payload, loadingFinishWithError: false }))
		const { deepLinkParams } = yield select(deepLinksSelector);
		const {
			accessToken,
			refreshToken
		}: IRegisterMethodResponse = yield Auth.register(payload, deepLinkParams);
		yield setTokenLocally(accessToken!, refreshToken!);
		navigationService.navigate(ScreensEnum.CHECK_EMAIL_SCREEN, { email: payload.email })
		//yield call(initAuthenticationFlowSaga);
		yield put(registerSuccess());
		yield put(resetRegistrationForm())
	} catch (ex) {
		yield put(updateRegistrationForm({ loadingFinishWithError: true }))
		yield put(registerError());
		errorHandler(i18n.t('register.failedAuthModal.headerText'), i18n.t('register.failedAuthModal.message'))
...
function* logoutUserSaga() {
	try {
		...
		yield AsyncStorage.setItem('onBoardingFinished', 'true');
		yield AsyncStorage.setItem('isFirstHomeScreenEntry', 'true');
		setCrumbizApiAuthorizationHeader(null)
		setAuthHeaders(null)
		yield all([
...
function* forgotPasswordSaga({ payload }: IForgotPasswordPayload) {
	try {
		const res = yield Auth.forgotPassword(payload.email);
		...
		yield put(forgotPasswordFailed(ex));
		// errorHandler(i18n.t('restorePassword.error.errorHeader'),i18n.t('restorePassword.error.errorText'))
	}
}

function* changePasswordSaga({
	payload: { NewPassword, change }
	...
	try {
		...
		errorHandler('Error occurred', 'Please try again in few minutes')
		//throw new Error(ex);
	}
}
...
function* confirmRegisterationSaga({ payload }: IConfirmRegPayload) {
	const { email, code, cb } = payload;
	try {
		// @ts-ignore
		const { succeeded } = Auth.confirmRegisteration(email, code);
...
export function* initAuthenticationFlowSaga() {
	try {
		...
		try {
			const responseIsEmailRegistered = yield authApi.get('/api/user/IsEmailRegistered');
			isEmailRegistered = responseIsEmailRegistered.data;
		} catch (error) {
			isEmailRegistered = false
		}
		if (!isEmailRegistered) {

			navigationService.navigate(ScreensEnum.CHECK_EMAIL_SCREEN)
		}
		yield setEmailValidation(isEmailRegistered);
...
export function* watchAuthSaga() {
	yield all([
		...
		takeLatest(AuthActionTypes.GET_EXISTS as any, getExistsSaga),
		takeLatest(
